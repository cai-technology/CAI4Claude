...
import { getAllClaudeDirs } from '../hosts.js'
...
export const claude: Provider = {
  ...
  async discoverSessions(): Promise<SessionSource[]> {
    ...
    // Iterate over all configured hosts (local + synced remotes via hosts.yaml)
    const allDirs = getAllClaudeDirs()
    for (const { host, path: claudeDir } of allDirs) {
      ...
      try {
        ...
        for (const dirName of entries) {
          ...
          if (dirStat?.isDirectory()) {
            // Prefix project name with [host] for clarity in multi-host reports
            const project = host === 'local' ? dirName : `[${host}] ${dirName}`
            sources.push({ path: dirPath, project, provider: 'claude', host })
          }
        }
      } catch {}
    }

    // Claude Desktop sessions (local-only, no multi-host support yet)
    const desktopDirs = await findDesktopProjectDirs(getDesktopSessionsDir())
    for (const dirPath of desktopDirs) {
      sources.push({ path: dirPath, project: basename(dirPath), provider: 'claude', host: 'local' })
    }
